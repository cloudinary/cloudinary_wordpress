...
class CloudinaryPlugin
{
    ...
    public function migrate_away($attach_id)
    {
        ...
        if ($response['response']['code'] != 200) {
            ...
            if (!$error) {
                $error = "Unable to migrate away $url";
            }
     ...
     */
    public function extract_metadata($remote_meta)
    {
     ...
     * @deprecated
     */
    public function extract_meta_value($info, $keys, $default = '')
    {
        trigger_error('Method '.__METHOD__.' is deprecated', E_USER_DEPRECATED);
        foreach ($keys as $key) {
            if (!empty($info[$key])) {
    ...
    /* Upgrade */
    public function cloudinary_upgrade()
    {
        $current_version = get_option('cloudinary_version');
        if (cloudinary_VERSION != $current_version) {
    ...
    /* Configure menus */
    public function cloudinary_options_page()
    {
    ...
    public function update_image_src_all($attachment_id, $attachment_metadata, $old_url, $new_url, $migrate_in, &$errors)
    {
        $query = new WP_Query(
      array(
        'post_type' => 'any',
    ...
    public function update_image_src($post, $attachment_id, $attachment_metadata, $old_url, $new_url, $migrate_in, &$errors)
    {
        $this->get_wp_sizes();
    ...
    public function init_media_lib_integration($xdmremote, $autoShow)
    {
        $cloudinary_js_dir = plugins_url('/js', __FILE__);
        wp_enqueue_script('jquery');
    ...
    public function media_cloudinary($editor_id = 'content')
    {
    ...
    public function media_lib_add_upload_column($cols)
    {
    ...
    public function media_lib_upload_column_value($column_name, $attachment_id)
    {
    ...
    public function media_lib_upload_admin_footer()
    {
        if (!$this->configured()) {
            return;
        }
    ...
    public function upload_to_cloudinary($attachment_id, $migrate)
    {
        $md = wp_get_attachment_metadata($attachment_id);
    ...
    public function media_lib_upload_action()
    {
        ...
        global $pagenow;
        if ('upload.php' == $pagenow && isset($_REQUEST['cloud_upload']) && (int) $_REQUEST['cloud_upload']) {
            if (!$this->configured()) {
        ...
        if ('upload_cloudinary' === $action || 'migrate_away_cloudinary' === $action) {
            if (!$this->configured()) {
                echo 'Please setup environment to upload images to cloudinary';
                exit();
            }
            ...
            foreach ($post_ids as $k => $post_id) {
                $error = 'upload_cloudinary' === $action ? $this->upload_to_cloudinary($post_id, true) : $this->migrate_away($post_id);
                if ($error) {
    ...
    public function return_to_media_lib($errors, $successes, $action, $sendback)
    {
        $image = 1 == $successes ? 'image' : 'images';
        $action_message = 'upload_cloudinary' === $action ? ' uploaded to cloudinary.' : ' migrated away from cloudinary.';
        $message = number_format_i18n($successes).' '.$image.$action_message;

        if (!empty($errors)) {
            $errors_count = array_sum($errors);
            $errors_count_message = 1 == $errors_count ? 'error' : 'errors';
    ...
    public function media_lib_upload_notices()
    {
        global $post_type, $pagenow;
        if ('upload.php' == $pagenow && 'attachment' == $post_type && Cloudinary::option_get($_REQUEST, 'cloudinary_message')) {
            $message = htmlentities($_REQUEST['cloudinary_message'], ENT_NOQUOTES);
            echo "<div class=\"updated\"><p>{$message}</p></div>";
        }
        if ('upload.php' == $pagenow && Cloudinary::option_get($_REQUEST, 'cloud_upload')) {
            $message = 'Sorry, this file format is not supported.';
            echo "<div class=\"updated\"><p>{$message}</p></div>";
        }
    }

    public function prepare_cloudinary_media_lib_url($mode)
    {
        if (!$this->configured()) {
            return null;
        }
