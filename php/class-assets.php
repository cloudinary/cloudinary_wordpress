...
class Assets extends Settings_Component {
	...
	public function create_edited_asset( $attachment_id ) {
		...
		$current = Utils::basename( get_attached_file( $attachment_id, true ) );
	...
	public function build_item( $item ) {
		...
		$return = array(
			...
			'data'            => $item,
			'base'            => $parts[0],
			'file'            => ! empty( $parts[1] ) ? $parts[1] : Utils::basename( $item['sized_url'] ),
			'size'            => $break,
	...
	protected function create_asset( $url, $parent_id ) {
		...
		$file_path = str_replace( home_url(), untrailingslashit( ABSPATH ), urldecode( $full_url ) );
		...
		$hash_name   = md5( $url );
		$wp_filetype = wp_check_filetype( Utils::basename( $url ), wp_get_mime_types() );
		$args        = array(
			'post_title'     => $url,
			'post_content'   => '',
			'post_name'      => $hash_name,
