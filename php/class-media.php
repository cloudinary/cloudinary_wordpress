...
class Media extends Settings_Component implements Setup {
	...
	public function get_asset_payload() {
		...
		$asset = array(
			...
			'public_id'       => filter_var( $data['asset']['public_id'], FILTER_CALLBACK, array( 'options' => 'sanitize_text_field' ) ),
			'type'            => filter_var( $data['asset']['type'], FILTER_CALLBACK, array( 'options' => 'sanitize_text_field' ) ),
			'format'          => filter_var( $data['asset']['format'], FILTER_CALLBACK, array( 'options' => 'sanitize_text_field' ) ),
		...
		if ( ! empty( $data['asset']['context'] ) ) {
			array_walk_recursive(
				...
				function ( $value, $key ) use ( &$asset ) {
					$asset['meta'][ $key ] = filter_var( $value, FILTER_CALLBACK, array( 'options' => 'sanitize_text_field' ) );
				}
			);
		}
	...
	public function down_sync_asset() {
		$nonce = filter_input( INPUT_POST, 'nonce', FILTER_CALLBACK, array( 'options' => 'sanitize_text_field' ) );
	...
	public function apply_media_library_filters( $query ) {
		if ( is_admin() && $query->is_main_query() ) {
			$request = filter_input( INPUT_GET, 'cloudinary-filter', FILTER_CALLBACK, array( 'options' => 'sanitize_text_field' ) );
